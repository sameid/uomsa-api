...
tests.EVAL_1 = function () {
    ...
    ***REMOVED***
    ...
    // test {EVAL - Lua integer -> Redis protocol type conversion***REMOVED***
    client.eval("return 100.5", 0, require_number(100, name));
    // test {EVAL - Lua string -> Redis protocol type conversion***REMOVED***
    client.eval("return 'hello world'", 0, require_string("hello world", name));
    // test {EVAL - Lua true boolean -> Redis protocol type conversion***REMOVED***
    client.eval("return true", 0, require_number(1, name));
    // test {EVAL - Lua false boolean -> Redis protocol type conversion***REMOVED***
    client.eval("return false", 0, require_null(name));
    // test {EVAL - Lua status code reply -> Redis protocol type conversion***REMOVED***
    client.eval("return {ok='fine'***REMOVED***", 0, require_string("fine", name));
    // test {EVAL - Lua error reply -> Redis protocol type conversion***REMOVED***
    client.eval("return {err='this is an error'***REMOVED***", 0, require_error(name));
    // test {EVAL - Lua table -> Redis protocol type conversion***REMOVED***
    client.eval("return {1,2,3,'ciao',{1,2***REMOVED******REMOVED***", 0, function (err, res) {
    ...
    ***REMOVED***);
    // test {EVAL - Are the KEYS and ARGS arrays populated correctly?***REMOVED***
    client.eval("return {KEYS[1],KEYS[2],ARGV[1],ARGV[2]***REMOVED***", 2, "a", "b", "c", "d", function (err, res) {
    ...
    ***REMOVED***);
    ...
    // test {EVAL - parameters in array format gives same result***REMOVED***
    client.eval(["return {KEYS[1],KEYS[2],ARGV[1],ARGV[2]***REMOVED***", 2, "a", "b", "c", "d"], function (err, res) {
        assert.strictEqual(4, res.length, name);
        assert.strictEqual("a", res[0], name);
        assert.strictEqual("b", res[1], name);
    ...
    ***REMOVED***);
    ...
    client.set("bulk reply key", "bulk reply value", function (err, res) {
        // test {EVAL - Redis bulk -> Lua type conversion***REMOVED***
        client.eval("local foo = redis.call('get','bulk reply key'); return {type(foo),foo***REMOVED***", 0, function (err, res) {
            if (err) throw err;
            assert.strictEqual(2, res.length, name);
    ...
    ***REMOVED***);
    ...
    // test {EVAL - Redis multi bulk -> Lua type conversion***REMOVED***
    client.multi()
        .del("mylist")
        .rpush("mylist", "a")
        .rpush("mylist", "b")
        ...
        .exec(function (err, replies) {
            if (err) throw err;
            client.eval("local foo = redis.call('lrange','mylist',0,-1); return {type(foo),foo[1],foo[2],foo[3],# foo***REMOVED***", 0, function (err, res) {
                assert.strictEqual(5, res.length, name);
            ...
            ***REMOVED***);
        ***REMOVED***);
    // test {EVAL - Redis status reply -> Lua type conversion***REMOVED***
    client.eval("local foo = redis.call('set','mykey','myval'); return {type(foo),foo['ok']***REMOVED***", 0, function (err, res) {
        if (err) throw err;
        assert.strictEqual(2, res.length, name);
        assert.strictEqual("table", res[0], name);
        assert.strictEqual("OK", res[1], name);
    ***REMOVED***);
    // test {EVAL - Redis error reply -> Lua type conversion***REMOVED***
    client.set("error reply key", "error reply value", function (err, res) {
        if (err) throw err;
        client.eval("local foo = redis.pcall('incr','error reply key'); return {type(foo),foo['err']***REMOVED***", 0, function (err, res) {
            if (err) throw err;
            assert.strictEqual(2, res.length, name);
            assert.strictEqual("table", res[0], name);
        ...
        ***REMOVED***);
    ***REMOVED***);
    // test {EVAL - Redis nil bulk reply -> Lua type conversion***REMOVED***
    client.del("nil reply key", function (err, res) {
        if (err) throw err;
        client.eval("local foo = redis.call('get','nil reply key'); return {type(foo),foo == false***REMOVED***", 0, function (err, res) {
            if (err) throw err;
            assert.strictEqual(2, res.length, name);
...
tests.HMGET = function () {
    ...
    ***REMOVED***, require_string("OK", name));
    ...
    ***REMOVED***), require_string("OK", name));
    ...
    client.KEYS("multibulk:*", function(err, results) {
        ...
        assert.deepEqual(keys_values.map(function(val) {
            return val[0];
        ***REMOVED***).sort(), results.sort(), name);
    ***REMOVED***);
...
tests.HGETALL_MESSAGE = function () {
    ...
    client.hmset("msg_test", {message: "hello"***REMOVED***, require_string("OK", name));
    ...
    ***REMOVED***);
    monitor_client.on("monitor", function (time, args) {
        ...
        if (responses.length === 2) {
            ...
            assert.strictEqual('{"foo":"123","bar":"sdflkdfsjk","another":false***REMOVED***', responses[1][2]);
    ...
    ***REMOVED***
    ...
    if (domain) {
        ...
        ***REMOVED***);
        ...
        domain.on('error', function (err) { next(name); ***REMOVED***);
    ***REMOVED***
***REMOVED***;
