...
Request.prototype.init = function (options) {
    ...
    if (http.globalAgent && self.uri.protocol === "https:") {
      ...
      var tunnelOptions = { proxy: { host: self.proxy.hostname
                                   ...
                                   , headers: { Host: self.uri.hostname + ':' +
                                        (self.uri.port || self.uri.protocol === 'https:' ? 443 : 80) ***REMOVED******REMOVED***
  ...
  ***REMOVED***
  ...
  if (!self.uri.pathname) {self.uri.pathname = '/'***REMOVED***
  if (!self.uri.port) {
    if (self.uri.protocol == 'http:') {self.uri.port = 80***REMOVED***
    else if (self.uri.protocol == 'https:') {self.uri.port = 443***REMOVED***
  ***REMOVED***
  ...
  ***REMOVED***
  ...
  if (self.uri.auth && !self.headers.authorization) {
    var authPieces = self.uri.auth.split(':').map(function(item){ return querystring.unescape(item) ***REMOVED***)
    self.auth(authPieces[0], authPieces.slice(1).join(':'), true)
  ***REMOVED***
  if (self.proxy && self.proxy.auth && !self.headers['proxy-authorization'] && !self.tunnel) {
    self.headers['proxy-authorization'] = "Basic " + toBase64(self.proxy.auth.split(':').map(function(item){ return querystring.unescape(item)***REMOVED***).join(':'))
  ***REMOVED***
  ...
  ***REMOVED*** else if (options.multipart) {
    self.boundary = uuid()
  ...
  ***REMOVED***
  ...
  var protocol = self.proxy && !self.tunnel ? self.proxy.protocol : self.uri.protocol
    , defaultModules = {'http:':http, 'https:':https***REMOVED***
    , httpModules = self.httpModules || {***REMOVED***
    ;
  ...
  if (!self.agent) {
    ...
    ***REMOVED*** else if (options.forever) {
  ...
  process.nextTick(function () {
    ...
    ***REMOVED***
    if (self.body) {
      ...
      ***REMOVED***
      self.end()
    ***REMOVED*** else if (self.requestBodyStream) {
      console.warn("options.requestBodyStream is deprecated, please pass the request object to stream.pipe.")
      self.requestBodyStream.pipe(self)
    ***REMOVED*** else if (!self.src) {
...
Request.prototype._updateProtocol = function () {
  ...
  if (protocol === 'https:') {
    ...
    if (self.proxy) {
      ...
      var tunnelOptions = { proxy: { host: self.proxy.hostname
                                   , port: +self.proxy.port
                                   , proxyAuth: self.proxy.auth ***REMOVED***
                          , rejectUnauthorized: self.rejectUnauthorized
                          , ca: self.ca ***REMOVED***
      self.agent = tunnelFn(tunnelOptions)
      return
...
Request.prototype.onResponse = function (response) {
  ...
  ***REMOVED***
  ...
  if (response.statusCode >= 300 && response.statusCode < 400 && response.headers.location) {
    ...
    ***REMOVED*** else if (self.followRedirect) {
      switch (self.method) {
        case 'PATCH':
        case 'PUT':
        case 'POST':
      ...
      ***REMOVED***
    ***REMOVED***
  ***REMOVED*** else if (response.statusCode == 401 && self._hasAuth && !self._sentAuth) {
  ...
  ***REMOVED*** else {
    ...
    ***REMOVED***)
    response.on("end", function (chunk) {
      ...
      self.emit("end", chunk)
    ***REMOVED***)
    response.on("close", function () {self.emit("close")***REMOVED***)
...
Request.prototype.setHeaders = function (headers) {
  for (var i in headers) {this.setHeader(i, headers[i])***REMOVED***
  return this
***REMOVED***
...
Request.prototype.aws = function (opts, now) {
  ...
  ***REMOVED*** else if (opts.bucket && !this.path) {
...
Request.prototype.oauth = function (_oauth) {
  ...
  ***REMOVED***
  ...
  this.headers.Authorization =
    'OAuth '+Object.keys(oa).sort().map(function (i) {return i+'="'+oauth.rfc3986(oa[i])+'"'***REMOVED***).join(',')
...
Request.prototype.jar = function (jar) {
  ...
  ***REMOVED*** else if (jar && jar.get) {
    // fetch cookie from the user defined cookie jar
    cookies = jar.get({ url: this.uri.href ***REMOVED***)
  ***REMOVED*** else {
    // fetch cookie from the global cookie jar
    cookies = cookieJar.get({ url: this.uri.href ***REMOVED***)
  ***REMOVED***
...
function initParams(uri, options, callback) {
  ...
  if (options && typeof options === 'object') {
    options.uri = uri
  ***REMOVED*** else if (typeof uri === 'string') {
    options = {uri:uri***REMOVED***
  ***REMOVED*** else {
    options = uri
    uri = options.uri
  ***REMOVED***
  return { uri: uri, options: options, callback: callback ***REMOVED***
***REMOVED***
...
function getSafe (self, uuid) {
  if (typeof self === 'object' || typeof self === 'function') var safe = {***REMOVED***
