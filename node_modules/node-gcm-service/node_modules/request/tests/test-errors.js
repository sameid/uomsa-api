var server = require('./server')
  , events = require('events')
  , assert = require('assert')
  , request = require('../index')
  ;

var local = 'http://localhost:8888/asdf'

try {
  request({uri:local, body:{***REMOVED******REMOVED***)
  assert.fail("Should have throw") 
***REMOVED*** catch(e) {
  assert.equal(e.message, 'Argument error, options.body.')
***REMOVED***

try {
  request({uri:local, multipart: 'foo'***REMOVED***)
  assert.fail("Should have throw")
***REMOVED*** catch(e) {
  assert.equal(e.message, 'Argument error, options.multipart.')
***REMOVED***

try {
  request({uri:local, multipart: [{***REMOVED***]***REMOVED***)
  assert.fail("Should have throw")
***REMOVED*** catch(e) {
  assert.equal(e.message, 'Body attribute missing in multipart.')
***REMOVED***

try {
  request(local, {multipart: [{***REMOVED***]***REMOVED***)
  assert.fail("Should have throw")
***REMOVED*** catch(e) {
  assert.equal(e.message, 'Body attribute missing in multipart.')
***REMOVED***

console.log("All tests passed.")
