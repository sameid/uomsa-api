...
# for i in {0..9***REMOVED***; do node benchmark.js >> bench_0.6.2.log; done;
...
for I in ${!FILES[*]***REMOVED***; do
  F=${FILES[$I]***REMOVED***
  P=${PATTERNS[$I]***REMOVED***
  INDEX=${INDICES[$I]***REMOVED***
  echo "version $F" > $F
  for V in $VERSIONS; do
    (VAL=$( grep "$P" bench_$V.log | LC_ALL=en_US awk '{ sum += $'$INDEX' ***REMOVED*** END { print sum/NR ***REMOVED***' ); echo $V $VAL) >> $F
  done
  if [ $I == 0 ]; then
    cat $F > $TMPJOIN
  else
