...
function test()
{
	...
	buffer[0] = 23;
	buffer[3] = 42;
	result = p.readData([ { x: { type: 'uint8_t' ***REMOVED******REMOVED***,
	    { y: { type: 'uint8_t', offset: 3 ***REMOVED******REMOVED***
	], buffer, 0);
	...
	result = p.readData([ { x: { type: 'char[20]' ***REMOVED******REMOVED*** ], buffer, 0);
	...
	buffer[2] = 0x25;
	buffer[3] = 0x26;
	result = p.readData([ { y: { type: 'uint8_t' ***REMOVED******REMOVED***,
	    { x: { type: 'uint8_t[y]' ***REMOVED******REMOVED***], buffer, 0);
	console.log(mod_sys.inspect(result, true));

	p.typedef('ssize_t', 'int32_t');
	ASSERT.deepEqual({ 'ssize_t': 'int32_t' ***REMOVED***, p.lstypes());
	result = p.readData([ { x: { type: 'ssize_t' ***REMOVED*** ***REMOVED*** ], buffer, 0);
