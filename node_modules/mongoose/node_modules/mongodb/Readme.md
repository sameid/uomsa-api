...
```javascript
  ...
  MongoClient.connect('mongodb://127.0.0.1:27017/test', function(err, db) {
    ...
    collection.insert({a:2***REMOVED***, function(err, docs) {
...
```javascript
  ...
  collection.findOne({_id: new ObjectID(idString)***REMOVED***, console.log)  // ok
  collection.findOne({_id: idString***REMOVED***, console.log)  // wrong! callback gets undefined
```
...
```javascript
  ...
  MongoClient.connect('mongodb://127.0.0.1:27017/test'
    , {db: {native_parser: true***REMOVED******REMOVED***, function(err, db) {***REMOVED***)
```
  ...
  MongoClient.connect('mongodb://127.0.0.1:27017/test', function(err, db) {
    ...
    db.dropDatabase(function(err, done) {
      ...
      db.createCollection('test_custom_key', function(err, collection) {
        ...
        collection.insert({'a':1***REMOVED***, function(err, docs) {
          
          collection.find({'_id':new ObjectID("aaaaaaaaaaaa")***REMOVED***).toArray(function(err, items) {
            console.dir(items);
            // Let's close the db
            db.close();
          ***REMOVED***);
...
```javascript
  ...
  cursor.nextObject(function(err, doc) {***REMOVED***);
  cursor.each(function(err, doc) {***REMOVED***);
...
with `batchSize(n, function(err){***REMOVED***)` after performing the initial query to the database.
...
```javascript
  ...
  MongoClient.connect('mongodb://127.0.0.1:27017/test', function(err, db) {
    ...
    db.collection('test').insert({hello: 'world'***REMOVED***, {w:1***REMOVED***, function(err, objects) {
...
```javascript
  ...
  MongoClient.connect('mongodb://127.0.0.1:27017/test', function(err, db) {
    ...
    db.collection('test').update({hi: 'here'***REMOVED***, {$set: {hi: 'there'***REMOVED******REMOVED***, {w:1***REMOVED***, function(err) {
      if (err) console.warn(err.message);
...
```javascript
  ...
  MongoClient.connect('mongodb://127.0.0.1:27017/test', function(err, db) {
    if(err) throw err;
    db.collection('test').findAndModify({hello: 'world'***REMOVED***, [['_id','asc']], {$set: {hi: 'there'***REMOVED******REMOVED***, {***REMOVED***, function(err, object) {
      if (err) console.warn(err.message);
