...
***REMOVED***
	...
	{
		Local<Value> argv[] = { returnObject->Get(bson->_dbRefRefString), returnObject->Get(bson->_dbRefIdRefString), returnObject->Get(bson->_dbRefDbRefString) ***REMOVED***;
	...
	case BSON_TYPE_TIMESTAMP:
		{
			...
			Local<Value> argv[] = { Int32::New(lowBits), Int32::New(highBits) ***REMOVED***;
	...
	case BSON_TYPE_CODE:
		{
			...
			Local<Value> argv[] = { code, scope ***REMOVED***;
	...
	***REMOVED***
	...
	case BSON_TYPE_BINARY:
		...
		***REMOVED***
			...
			Handle<Value> argv[] = { buffer->handle_, Uint32::New(subType) ***REMOVED***;
	...
	case BSON_TYPE_SYMBOL:
		{
			...
			Local<Value> argv[] = { string ***REMOVED***;
		...
		if(array->Length() > 0)
		{
			...
			for(uint32_t i = 0; i < array->Length(); i++) {
				...
				if(functionName->StrictEquals(bson->longString)) {
					...
					foundClassesMask |= 1;
			***REMOVED*** else if(functionName->StrictEquals(bson->objectIDString)) {
					bson->objectIDConstructor = Persistent<Function>::New(func);
					foundClassesMask |= 2;
			***REMOVED*** else if(functionName->StrictEquals(bson->binaryString)) {
					bson->binaryConstructor = Persistent<Function>::New(func);
					foundClassesMask |= 4;
			***REMOVED*** else if(functionName->StrictEquals(bson->codeString)) {
					bson->codeConstructor = Persistent<Function>::New(func);
					foundClassesMask |= 8;
			***REMOVED*** else if(functionName->StrictEquals(bson->dbrefString)) {
					bson->dbrefConstructor = Persistent<Function>::New(func);
					foundClassesMask |= 0x10;
			***REMOVED*** else if(functionName->StrictEquals(bson->symbolString)) {
					bson->symbolConstructor = Persistent<Function>::New(func);
					foundClassesMask |= 0x20;
			***REMOVED*** else if(functionName->StrictEquals(bson->doubleString)) {
					bson->doubleConstructor = Persistent<Function>::New(func);
					foundClassesMask |= 0x40;
			***REMOVED*** else if(functionName->StrictEquals(bson->timestampString)) {
					bson->timestampConstructor = Persistent<Function>::New(func);
					foundClassesMask |= 0x80;
			***REMOVED*** else if(functionName->StrictEquals(bson->minKeyString)) {
					bson->minKeyConstructor = Persistent<Function>::New(func);
					foundClassesMask |= 0x100;
			***REMOVED*** else if(functionName->StrictEquals(bson->maxKeyString)) {
