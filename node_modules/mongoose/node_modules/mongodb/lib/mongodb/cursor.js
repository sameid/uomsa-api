 ...
 *
 * Options
 *  - **skip** {Number***REMOVED*** skip number of documents to skip.
 *  - **limit** {Number***REMOVED***, limit the number of results to return. -1 has a special meaning and is used by Db.eval. A value of 1 will also be treated as if it were -1.
 *  - **sort** {Array | Object***REMOVED***, set to sort the documents coming back from the query. Array of indexes, [['a', 1]] etc.
 *  - **hint**  {Object***REMOVED***, hint force the query to use a specific index.
 *  - **explain** {Boolean***REMOVED***, explain return the explaination of the query.
 *  - **snapshot** {Boolean***REMOVED***, snapshot Snapshot mode assures no duplicates are returned.
 *  - **timeout** {Boolean***REMOVED***, timeout allow the query to timeout.
 *  - **tailable** {Boolean***REMOVED***, tailable allow the cursor to be tailable.
 *  - **awaitdata** {Boolean***REMOVED***, awaitdata allow the cursor to wait for data, only applicable for tailable cursor.
 *  - **batchSize** {Number***REMOVED***, batchSize the number of the subset of results to request the database to return for every request. This should initially be greater than 1 otherwise the database will automatically close the cursor. The batch size can be set to 1 with cursorInstance.batchSize after performing the initial query to the database.
 *  - **raw** {Boolean***REMOVED***, raw return all query documents as raw buffers (default false).
 *  - **read** {Boolean***REMOVED***, read specify override of read from source (primary/secondary).
 *  - **returnKey** {Boolean***REMOVED***, returnKey only return the index key.
 *  - **maxScan** {Number***REMOVED***, maxScan limit the number of items to scan.
 *  - **min** {Number***REMOVED***, min set index bounds.
 *  - **max** {Number***REMOVED***, max set index bounds.
 *  - **showDiskLoc** {Boolean***REMOVED***, showDiskLoc show disk location of results.
 *  - **comment** {String***REMOVED***, comment you can put a $comment field on a query to make looking in the profiler logs simpler.
 *  - **numberOfRetries** {Number***REMOVED***, numberOfRetries if using awaidata specifies the number of times to retry on timeout.
 *  - **dbName** {String***REMOVED***, dbName override the default dbName.
 *  - **tailableRetryInterval** {Number***REMOVED***, tailableRetryInterval specify the miliseconds between getMores on tailable cursor.
 *  - **exhaust** {Boolean***REMOVED***, exhaust have the server send all the documents at once as getMore packets.
 *  - **partial** {Boolean***REMOVED***, partial have the sharded system return a partial result from mongos.
 *
 * @class Represents a Cursor.
 * @param {Db***REMOVED*** db the database object to work with.
 * @param {Collection***REMOVED*** collection the collection to query.
 * @param {Object***REMOVED*** selector the query selector.
 * @param {Object***REMOVED*** fields an object containing what fields to include or exclude from objects returned.
...
function Cursor(db, collection, selector, fields, options) {
  ...
  this.fields = fields;
  options = !options ? {***REMOVED*** : options;
 ...
 *
 * @return {Cursor***REMOVED*** returns itself with rewind applied.
 * @api public
 */
 ...
 *
 * @param {Function***REMOVED*** callback This will be called after executing this method successfully. The first parameter will contain the Error object if an error occured, or null otherwise. The second parameter will contain an array of BSON deserialized objects as a result of the query.
 * @return {null***REMOVED***
 * @api public
 */
Cursor.prototype.toArray = function(callback) {
  ...
  ***REMOVED*** else if(this.state != Cursor.CLOSED) {
    ...
    self.nextObject({noReturn: true***REMOVED***, function(err, result) {
...
var getAllByGetMore = function(self, callback) {
  getMore(self, {noReturn: true***REMOVED***, function(err, result) {
 ...
 *
 * @param {Function***REMOVED*** callback this will be called for while iterating every document of the query result. The first parameter will contain the Error object if an error occured, or null otherwise. While the second parameter will contain the document.
 * @return {null***REMOVED***
 * @api public
 */
 ...
 *
 * @param {Boolean***REMOVED*** applySkipLimit if set to true will apply the skip and limits set on the cursor. Defaults to false.
 * @param {Function***REMOVED*** callback this will be called after executing this method. The first parameter will contain the Error object if an error occured, or null otherwise. While the second parameter will contain the number of results or null if an error occured.
 * @return {null***REMOVED***
 * @api public
 */
 ...
 *
 * @param {String|Array|Object***REMOVED*** keyOrList This can be a string or an array. If passed as a string, the string will be the field to sort. If passed an array, each element will represent a field to be sorted and should be an array that contains the format [string, direction].
 * @param {String|Number***REMOVED*** direction this determines how the results are sorted. "asc", "ascending" or 1 for asceding order while "desc", "desceding or -1 for descending order. Note that the strings are case insensitive.
 * @param {Function***REMOVED*** callback this will be called after executing this method. The first parameter will contain an error object when the cursor is already closed while the second parameter will contain a reference to this object upon successful execution.
 * @return {Cursor***REMOVED*** an instance of this object.
 * @api public
 */
Cursor.prototype.sort = function(keyOrList, direction, callback) {
  callback = callback || function(){***REMOVED***;
  if(typeof direction === "function") { callback = direction; direction = null; ***REMOVED***

  if(this.tailable) {
    callback(new Error("Tailable cursor doesn't support sorting"), null);
  ***REMOVED*** else if(this.queryRun == true || this.state == Cursor.CLOSED) {
    callback(new Error("Cursor is closed"), null);
  ***REMOVED*** else {
    var order = keyOrList;
 ...
 *
 * @param {String***REMOVED*** the read preference for the cursor, one of Server.READ_PRIMARY, Server.READ_SECONDARY, Server.READ_SECONDARY_ONLY
...
Cursor.prototype.setReadPreference = function(readPreference, tags, callback) {
  ...
  ***REMOVED*** else if(_mode != null && _mode != 'primary'
 ...
 *
 * @param {Number***REMOVED*** skip the new skip value.
 ...
 *
 * @param {Number***REMOVED*** batchSize the new batch size.
...
Cursor.prototype.batchSize = function(batchSize, callback) {
  if(this.state == Cursor.CLOSED) {
    ...
    ***REMOVED*** else {
      throw new Error("Cursor is closed");
    ***REMOVED***
  ***REMOVED*** else if(batchSize != null && batchSize.constructor != Number) {
    if(callback != null) {
 ...
 *
 * @return {Number***REMOVED*** The number of records to request per batch.
 * @ignore
 * @api private
 */
 ...
 *
 * @return {QueryCommand***REMOVED*** The command object
 * @ignore
 * @api private
 */
var generateQueryCommand = function(self) {
  ...
  ***REMOVED***
  ...
  if(self.sortValue != null || self.explainValue != null || self.hint != null || self.snapshot != null
    ...
    var specialSelector = {'$query':self.selector***REMOVED***;
...
/**
 * @return {Object***REMOVED*** Returns an object containing the sort value of this cursor with
 ...
 *
 * @param sortDirection {String|number***REMOVED*** Range of acceptable values:
 *     'ascending', 'descending', 'asc', 'desc', 1, -1
 *
 * @return {number***REMOVED*** The equivalent numerical value
 ...
 *
 * @param {Function***REMOVED*** callback this will be called after executing this method. The first parameter will contain an error object on error while the second parameter will contain a document from the returned result or null if there are no more results.
 * @api public
 */
  ...
  ***REMOVED***
  ...
  if(self.state == Cursor.INIT) {
    ...
    ***REMOVED***
    ...
    var queryOptions = {exhaust: self.exhaust, raw:self.raw, read:self.read, connection:self.connection***REMOVED***;
    // Execute command
    var commandHandler = function(err, result) {
      ...
      if(err == null && (result == null || result.documents == null || !Array.isArray(result.documents))) {
        return self.close(function() {callback(new Error("command failed to return results"), null);***REMOVED***);
      ***REMOVED***

      if(err == null && result && result.documents[0] && result.documents[0]['$err']) {
        return self.close(function() {callback(utils.toError(result.documents[0]['$err']), null);***REMOVED***);
      ***REMOVED***
      ...
      ***REMOVED*** else if(self.exhaust == false || self.exhaust == null) {
        self.nextObject(callback);
      ***REMOVED***
    ***REMOVED***;
    ...
    ***REMOVED***
    ...
    commandHandler = null;
  ***REMOVED*** else if(self.items.length) {
    callback(null, self.items.shift());
  ***REMOVED*** else if(self.cursorId.greaterThan(Long.fromInt(0))) {
    getMore(self, callback);
  ***REMOVED*** else {
    // Force cursor to stay open
    return self.close(function() {callback(null, null);***REMOVED***);
  ***REMOVED***
***REMOVED***
...
var getMore = function(self, options, callback) {
  ...
  ***REMOVED***
  ...
  if (!self.tailable && self.limitValue > 0) {
    ...
    if (limit < 1) {
      self.close(function() {callback(null, null);***REMOVED***);
      return;
    ***REMOVED***
  ***REMOVED***
  ...
  try {
    ...
    // Set up options
    var command_options = {read: self.read, raw: self.raw, connection:self.connection ***REMOVED***;
      ...
      ***REMOVED***
      ...
      try {
        ...
        if(result.numberReturned > 0) {
          ...
          ***REMOVED***          
        ***REMOVED*** else if(self.tailable && !isDead && self.awaitdata) {
          ...
          ***REMOVED*** else {
            getMore(self, callback);
          ***REMOVED***
        ***REMOVED*** else if(self.tailable && !isDead) {
          self.getMoreTimer = setTimeout(function() { getMore(self, callback); ***REMOVED***, self.tailableRetryInterval);
        ***REMOVED*** else {
          self.close(function() {callback(null, null); ***REMOVED***);
        ***REMOVED***
 ...
 *
 * @param {Function***REMOVED*** callback this will be called after executing this method. The first parameter will always be null while the second parameter will be an object containing the details.
 * @api public
 */
 ...
 *
 * Options
 *  - **transform** {Function***REMOVED*** function of type function(object) { return transformed ***REMOVED***, allows for transformation of data before emitting.
 *
 * @return {CursorStream***REMOVED*** returns a stream object.
 * @api public
 */
 ...
 * Close the cursor.
 *
 * @param {Function***REMOVED*** callback this will be called after executing this method. The first parameter will always contain null while the second parameter will contain a reference to this cursor.
 * @return {null***REMOVED***
 * @api public
 */
Cursor.prototype.close = function(callback) {
  ...
  if(this.cursorId instanceof Long && this.cursorId.greaterThan(Long.fromInt(0))) {
    try {
      ...
      this.db._executeQueryCommand(command, {read:self.read, raw:self.raw, connection:self.connection***REMOVED***);
 ...
 *
 * @return {Boolean***REMOVED*** returns the state of the cursor.
 * @api public
 */
