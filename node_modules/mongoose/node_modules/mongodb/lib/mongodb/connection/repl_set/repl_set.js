 ...
 *
 * Options
 *  - **ha** {Boolean, default:true***REMOVED***, turn on high availability.
 *  - **haInterval** {Number, default:2000***REMOVED***, time between each replicaset status check.
 *  - **reconnectWait** {Number, default:1000***REMOVED***, time to wait in miliseconds before attempting reconnect.
 *  - **retries** {Number, default:30***REMOVED***, number of times to attempt a replicaset reconnect.
 *  - **rs_name** {String***REMOVED***, the name of the replicaset to connect to.
 *  - **socketOptions** {Object, default:null***REMOVED***, an object containing socket options to use (noDelay:(boolean), keepAlive:(number), connectTimeoutMS:(number), socketTimeoutMS:(number))
 ...
 *  - **strategy** {String, default:'ping'***REMOVED***, selection strategy for reads choose between (ping, statistical and none, default is ping)
 *  - **secondaryAcceptableLatencyMS** {Number, default:15***REMOVED***, sets the range of servers to pick when using NEAREST (lowest ping ms + the latency fence, ex: range of 1 to (1 + 15) ms)
 *  - **connectWithNoPrimary** {Boolean, default:false***REMOVED***, sets if the driver should connect even if no primary is available
 *  - **connectArbiter** {Boolean, default:false***REMOVED***, sets if the driver should connect to arbiters or not.
 ...
 *  - **poolSize** {Number, default:5***REMOVED***, number of connections in the connection pool for each server instance, set to 5 as default for legacy reasons.
 *  - **ssl** {Boolean, default:false***REMOVED***, use ssl connection (needs to have a mongod server with ssl support)
 *  - **sslValidate** {Boolean, default:false***REMOVED***, validate mongod server certificate against ca (needs to have a mongod server with ssl support, 2.4 or higher)
 *  - **sslCA** {Array, default:null***REMOVED***, Array of valid certificates either as Buffers or Strings (needs to have a mongod server with ssl support, 2.4 or higher)
 *  - **sslCert** {Buffer/String, default:null***REMOVED***, String or buffer containing the certificate we wish to present (needs to have a mongod server with ssl support, 2.4 or higher)
 *  - **sslKey** {Buffer/String, default:null***REMOVED***, String or buffer containing the certificate private key we wish to present (needs to have a mongod server with ssl support, 2.4 or higher)
 *  - **sslPass** {Buffer/String, default:null***REMOVED***, String or buffer containing the certificate password (needs to have a mongod server with ssl support, 2.4 or higher)
 *
 * @class Represents a 
 Replicaset Configuration
 * @param {Array***REMOVED*** list of server objects participating in the replicaset.
 * @param {Object***REMOVED*** [options] additional options for the replicaset connection.
 */
var ReplSet = exports.ReplSet = function(servers, options) {
  ...
  ***REMOVED*** else if(this.options.strategy == 'statistical') {
 ...
 *
 * @param {String***REMOVED*** host - host:port pair (localhost:27017)
 * @param {ReplSet***REMOVED*** replset - the ReplSet instance
 * @return {Server***REMOVED***
 * @ignore
 */
var createServer = function(self, host, options) {
  // copy existing socket options to new server
  var socketOptions = {***REMOVED***
...
var _handler = function(event, self, server) {
  return function(err, doc) {
    ...
    if(self._state.isPrimary(server)) {
      ...
      ***REMOVED***
    ***REMOVED*** else if(self._state.isSecondary(server)) {
...
var _connectHandler = function(self, candidateServers, instanceServer) {
  return function(err, doc) {
    ...
    ***REMOVED*** else if(candidateServers.length == 0) {      
...
ReplSet.prototype.isConnected = function(read) {
  ...
  if((read == ReadPreference.PRIMARY_PREFERRED || read == ReadPreference.SECONDARY_PREFERRED || read == ReadPreference.NEAREST)
    ...
    || (this._state && this._state.secondaries && Object.keys(this._state.secondaries).length > 0))) {
      isConnected = true;
  ***REMOVED*** else if(read == ReadPreference.SECONDARY) {
...
ReplSet.prototype.checkoutReader = function(readPreference, tags) {
  ...
  ***REMOVED*** else if(typeof readPreference == 'object' && readPreference['_type'] != 'ReadPreference') {
  ...
  if((this.options.readSecondary || finalReadPreference == ReadPreference.SECONDARY_PREFERRED || finalReadPreference == ReadPreference.SECONDARY) && Object.keys(this._state.secondaries).length > 0) {
    ...
    if(this.strategyInstance != null) {
      ...
      ***REMOVED*** else {
        ...
        ***REMOVED***
      ***REMOVED***
    ***REMOVED*** else if(tags != null && typeof tags == 'object') {
      // Get connection
    ...
    ***REMOVED***
  ***REMOVED*** else if(finalReadPreference == ReadPreference.PRIMARY_PREFERRED) {
    ...
    if(connection == null || connection instanceof Error) {
      ...
      ***REMOVED***
    ***REMOVED***
  ***REMOVED*** else if(finalReadPreference == ReadPreference.SECONDARY_PREFERRED) {
    // If we have tags, look for servers matching the specific tag
    if(this.strategyInstance != null) {
    ...
    ***REMOVED*** else if(tags != null && typeof tags == 'object') {
      ...
      ***REMOVED***
    ***REMOVED***
  ***REMOVED*** else if(finalReadPreference == ReadPreference.NEAREST && this.strategyInstance != null) {
    connection = this.strategyInstance.checkoutConnection(tags);
  ***REMOVED*** else if(finalReadPreference == ReadPreference.NEAREST && this.strategyInstance == null) {
    return new Error("A strategy for calculating nearness must be enabled such as ping or statistical");
  ***REMOVED*** else if(finalReadPreference == ReadPreference.SECONDARY && Object.keys(this._state.secondaries).length == 0) {
    if(tags != null && typeof tags == 'object') {
      var preferenceName = finalReadPreference == ReadPreference.SECONDARY ? 'secondary' : finalReadPreference;
      return new Error("No replica set member available for query with ReadPreference " + preferenceName + " and tags " + JSON.stringify(tags));
    ***REMOVED*** else {
