 ...
 *
 * Options
 *  - **w**, {Number/String, > -1 || 'majority' || tag name***REMOVED*** the write concern for the operation where < 1 is no acknowlegement of write and w >= 1, w = 'majority' or tag acknowledges the write
 *  - **wtimeout**, {Number, 0***REMOVED*** set the timeout for waiting for write concern to finish (combines with w option)
 *  - **fsync**, (Boolean, default:false) write waits for fsync before returning
 *  - **journal**, (Boolean, default:false) write waits for journal sync before returning
 *  - **readPreference** {String***REMOVED***, the prefered read preference (ReadPreference.PRIMARY, ReadPreference.PRIMARY_PREFERRED, ReadPreference.SECONDARY, ReadPreference.SECONDARY_PREFERRED, ReadPreference.NEAREST).
 *  - **native_parser** {Boolean, default:false***REMOVED***, use c++ bson parser.
 *  - **forceServerObjectId** {Boolean, default:false***REMOVED***, force server to create _id fields instead of client.
 *  - **pkFactory** {Object***REMOVED***, object overriding the basic ObjectID primary key generation.
 *  - **serializeFunctions** {Boolean, default:false***REMOVED***, serialize functions.
 *  - **raw** {Boolean, default:false***REMOVED***, peform operations using raw bson buffers.
 *  - **recordQueryStats** {Boolean, default:false***REMOVED***, record query statistics during execution.
 *  - **retryMiliSeconds** {Number, default:5000***REMOVED***, number of miliseconds between retries.
 *  - **numberOfRetries** {Number, default:5***REMOVED***, number of retries off connection.
 *  - **logger** {Object, default:null***REMOVED***, an object representing a logger that you want to use, needs to support functions debug, log, error **({error:function(message, object) {***REMOVED***, log:function(message, object) {***REMOVED***, debug:function(message, object) {***REMOVED******REMOVED***)**.
 *  - **slaveOk** {Number, default:null***REMOVED***, force setting of SlaveOk flag on queries (only use when explicitly connecting to a secondary server).
 *  - **promoteLongs** {Boolean, default:true***REMOVED***, when deserializing a Long will fit it into a Number if it's smaller than 53 bits
 * 
 * Deprecated Options 
 *  - **safe** {true | {w:n, wtimeout:n***REMOVED*** | {fsync:true***REMOVED***, default:false***REMOVED***, executes with a getLastError command returning the results of the command on MongoDB.
 *
 * @class Represents a Db
 * @param {String***REMOVED*** databaseName name of the database.
 * @param {Object***REMOVED*** serverConfig server config object.
 * @param {Object***REMOVED*** [options] additional options for the collection.
 */
function Db(databaseName, serverConfig, options) {
  ...
  this.options = options == null ? {***REMOVED*** : options;
  ...
  ***REMOVED*** else if(!overrideUsedFlag && typeof this.serverConfig == 'object'){
    // Set being used
  ...
  ***REMOVED***
  ...
  if(this.options.safe == null && this.options.w == null && this.options.journal == null && this.options.fsync == null) {
    ...
    console.log("=   allows values of [true | false | {j:true***REMOVED*** | {w:n, wtimeout:n***REMOVED*** | {fsync:true***REMOVED***]      =");
    ...
    console.log("=                                                                                      =");
    console.log("=   ex: new Db(new Server('localhost', 27017), {safe:false***REMOVED***)                           =");
    console.log("=                                                                                      =");
  ...
  ***REMOVED***
  ...
  this.logger = this.options.logger != null
    ...
    && (typeof this.options.logger.log == 'function')
      ? this.options.logger : {error:function(message, object) {***REMOVED***, log:function(message, object) {***REMOVED***, debug:function(message, object) {***REMOVED******REMOVED***;
  ...
  this.eventHandlers = {error:[], parseError:[], poolReady:[], message:[], close:[]***REMOVED***;
 ...
 *
 * @param {Function***REMOVED*** callback this will be called after executing this method. The first parameter will contain the Error object if an error occured, or null otherwise. While the second parameter will contain the index information or null if an error occured.
 * @return {null***REMOVED***
 * @api public
 */
 ...
 *
 * @param {String***REMOVED*** dbName the name of the database we want to use.
 * @return {Db***REMOVED*** a db instance using the new database.
 * @api public
 */
 ...
 *
 * @param {Boolean***REMOVED*** [forceClose] connection can never be reused.
 * @param {Function***REMOVED*** callback this will be called after executing this method. The first parameter will contain the Error object if an error occured, or null otherwise. While the second parameter will contain the results or null if an error occured.
 * @return {null***REMOVED***
 * @api public
 */
Db.prototype.close = function(forceClose, callback) {
  ...
  if(typeof forceClose == 'function') {
    callback = forceClose;
  ***REMOVED*** else if(typeof forceClose == 'boolean') {
 ...
 *
 * @param {Function***REMOVED*** [callback] returns the results.
 * @return {Admin***REMOVED*** the admin db object.
 * @api public
 */
 ...
 *
 * @param {String***REMOVED*** [collectionName] the collection name we wish to retrieve the information from.
 * @param {Function***REMOVED*** callback this will be called after executing this method. The first parameter will contain the Error object if an error occured, or null otherwise. While the second parameter will contain the options or null if an error occured.
 * @return {null***REMOVED***
 * @api public
 */
Db.prototype.collectionsInfo = function(collectionName, callback) {
  if(callback == null && typeof collectionName == 'function') { callback = collectionName; collectionName = null; ***REMOVED***
  // Create selector
 ...
 *
 * Options
 *  - **namesOnly** {String, default:false***REMOVED***, Return only the full collection namespace.
 *
 * @param {String***REMOVED*** [collectionName] the collection name we wish to filter by.
 * @param {Object***REMOVED*** [options] additional options during update.
 * @param {Function***REMOVED*** callback this will be called after executing this method. The first parameter will contain the Error object if an error occured, or null otherwise. While the second parameter will contain the collection names or null if an error occured.
 * @return {null***REMOVED***
 * @api public
 */
    ...
    cursor.toArray(function(err, documents) {
      ...
      ***REMOVED***);
      ...
      if(options.namesOnly) {
        filtered_documents = filtered_documents.map(function(document) { return document.name ***REMOVED***);
      ***REMOVED***
 ...
 *  - **fsync**, (Boolean, default:false) write waits for fsync before returning
 *  - **journal**, (Boolean, default:false) write waits for journal sync before returning
 *  - **serializeFunctions** {Boolean, default:false***REMOVED***, serialize functions on the document.
 *  - **raw** {Boolean, default:false***REMOVED***, perform all operations using raw bson objects.
 *  - **pkFactory** {Object***REMOVED***, object overriding the basic ObjectID primary key generation.
 *  - **readPreference** {String***REMOVED***, the prefered read preference (ReadPreference.PRIMARY, ReadPreference.PRIMARY_PREFERRED, ReadPreference.SECONDARY, ReadPreference.SECONDARY_PREFERRED, ReadPreference.NEAREST).
 ...
 *  - **safe** {true | {w:n, wtimeout:n***REMOVED*** | {fsync:true***REMOVED***, default:false***REMOVED***, executes with a getLastError command returning the results of the command on MongoDB.
 *
 * @param {String***REMOVED*** collectionName the collection name we wish to access.
 * @param {Object***REMOVED*** [options] returns option results.
 * @param {Function***REMOVED*** callback this will be called after executing this method. The first parameter will contain the Error object if an error occured, or null otherwise. While the second parameter will contain the collection or null if an error occured.
 * @return {null***REMOVED***
 * @api public
 */
 ...
 *
 * @param {Function***REMOVED*** callback this will be called after executing this method. The first parameter will contain the Error object if an error occured, or null otherwise. While the second parameter will contain the collections or null if an error occured.
 * @return {null***REMOVED***
 * @api public
 */
 ...
 *
 * Options
 *  - **nolock** {Boolean, default:false***REMOVED***, Tell MongoDB not to block on the evaulation of the javascript.
 *
 ...
 * @param {Function***REMOVED*** callback this will be called after executing this method. The first parameter will contain the Error object if an error occured, or null otherwise. While the second parameter will contain the results from eval or null if an error occured.
 * @return {null***REMOVED***
 * @api public
 */
Db.prototype.eval = function(code, parameters, options, callback) {
  ...
  ***REMOVED*** else if(parameters != null && parameters.constructor == Array && typeof parameters !== 'function') {
    finalParameters = parameters;
  ***REMOVED***

  // Create execution selector
  var selector = {'$eval':finalCode, 'args':finalParameters***REMOVED***;
 ...
 *
 * @param {DBRef***REMOVED*** dbRef db reference object we wish to resolve.
 * @param {Function***REMOVED*** callback this will be called after executing this method. The first parameter will contain the Error object if an error occured, or null otherwise. While the second parameter will contain the results from dereference or null if an error occured.
 * @return {null***REMOVED***
 * @api public
 */
Db.prototype.dereference = function(dbRef, callback) {
  ...
  collection.findOne({'_id':dbRef.oid***REMOVED***, function(err, result) {
    callback(err, result);
  ***REMOVED***);
***REMOVED***;
 ...
 *
 * @param {Function***REMOVED*** callback this will be called after executing this method. The first parameter will contain the Error object if an error occured, or null otherwise. While the second parameter will contain the results from logout or null if an error occured.
 * @return {null***REMOVED***
 * @api public
 */
Db.prototype.logout = function(options, callback) {
  ...
  var logoutCommand = DbCommand.logoutCommand(self, {logout:1***REMOVED***, options);
  self._executeQueryCommand(logoutCommand, {onAll:true***REMOVED***, function(err, result) {
    // Count down
 ...
 * authMechanism
 * Options
 *  - **authSource** {String***REMOVED***, The database that the credentials are for,
 *    different from the name of the current DB, for example admin
 *  - **authMechanism** {String, default:MONGODB-CR***REMOVED***, The authentication mechanism to use, GSSAPI or MONGODB-CR
 *
 * @param {String***REMOVED*** username username.
 * @param {String***REMOVED*** password password.
 * @param {Object***REMOVED*** [options] the options
 * @param {Function***REMOVED*** callback this will be called after executing this method. The first parameter will contain the Error object if an error occured, or null otherwise. While the second parameter will contain the results from authentication or null if an error occured.
 * @return {null***REMOVED***
 * @api public
 */
Db.prototype.authenticate = function(username, password, options, callback) {
  ...
  ***REMOVED*** else if(options.authMechanism == 'PLAIN') {
 ...
 * @param {String***REMOVED*** password password.
 * @param {Object***REMOVED*** [options] additional options during update.
 * @param {Function***REMOVED*** callback this will be called after executing this method. The first parameter will contain the Error object if an error occured, or null otherwise. While the second parameter will contain the results from addUser or null if an error occured.
 * @return {null***REMOVED***
 * @api public
 */
Db.prototype.addUser = function(username, password, options, callback) {
  ...
  collection.count({***REMOVED***, function(err, count) {
    // We got an error (f.ex not authorized)
    if(err != null) return callback(err, null);
    // Check if the user exists and update i
    collection.find({user: username***REMOVED***, {dbName: options['dbName']***REMOVED***).toArray(function(err, documents) {
      // We got an error (f.ex not authorized)
      if(err != null) return callback(err, null);
      // Add command keys
      ...
      collection.update({user: username***REMOVED***,{$set: {user: username, pwd: userPassword***REMOVED******REMOVED***, commandOptions, function(err, results) {
        if(count == 0 && err) {
          callback(null, [{user:username, pwd:userPassword***REMOVED***]);
        ***REMOVED*** else if(err) {
          callback(err, null)
        ***REMOVED*** else {
          callback(null, [{user:username, pwd:userPassword***REMOVED***]);
 ...
 * @param {String***REMOVED*** username username.
 * @param {Object***REMOVED*** [options] additional options during update.
 * @param {Function***REMOVED*** callback this will be called after executing this method. The first parameter will contain the Error object if an error occured, or null otherwise. While the second parameter will contain the results from removeUser or null if an error occured.
 * @return {null***REMOVED***
 * @api public
 */
Db.prototype.removeUser = function(username, options, callback) {
  ...
  var safe = self.safe != null && self.safe == false ? {w: 1***REMOVED*** : self.safe;
  ...
  safe = safe == null ? {w: 1***REMOVED*** : safe;

  // Fetch a user collection
  var collection = this.collection(DbCommand.SYSTEM_USER_COLLECTION);
  collection.findOne({user: username***REMOVED***, {dbName: options['dbName']***REMOVED***, function(err, user) {
    if(user != null) {
      // Add command keys
      var commandOptions = safe;
      commandOptions.dbName = options['dbName'];

      collection.remove({user: username***REMOVED***, commandOptions, function(err, result) {
        callback(err, true);
      ***REMOVED***);
    ***REMOVED*** else {
      callback(err, false);
 ...
 *  - **raw** {Boolean, default:false***REMOVED***, perform all operations using raw bson objects.
 *  - **pkFactory** {Object***REMOVED***, object overriding the basic ObjectID primary key generation.
 *  - **capped** {Boolean, default:false***REMOVED***, create a capped collection.
 *  - **size** {Number***REMOVED***, the size of the capped collection in bytes.
 *  - **max** {Number***REMOVED***, the maximum number of documents in the capped collection.
 *  - **autoIndexId** {Boolean, default:true***REMOVED***, create an index on the _id field of the document, True by default on MongoDB 2.2 or higher off for version < 2.2.
 *  - **readPreference** {String***REMOVED***, the prefered read preference (ReadPreference.PRIMARY, ReadPreference.PRIMARY_PREFERRED, ReadPreference.SECONDARY, ReadPreference.SECONDARY_PREFERRED, ReadPreference.NEAREST).
 ...
 * @param {String***REMOVED*** collectionName the collection name we wish to access.
 * @param {Object***REMOVED*** [options] returns option results.
 * @param {Function***REMOVED*** callback this will be called after executing this method. The first parameter will contain the Error object if an error occured, or null otherwise. While the second parameter will contain the results from createCollection or null if an error occured.
 * @return {null***REMOVED***
 * @api public
 */
 ...
 *
 * @param {Object***REMOVED*** selector the command hash to send to the server, ex: {ping:1***REMOVED***.
 * @param {Function***REMOVED*** callback this will be called after executing this method. The command always return the whole result of the command as the second parameter.
 * @return {null***REMOVED***
 * @api public
 */
Db.prototype.command = function(selector, options, callback) {
  ...
  var cursor = new Cursor(this
    , new Collection(this, DbCommand.SYSTEM_COMMAND_COLLECTION), selector, {***REMOVED***, {
  ...
  if(readPreference != false && ignoreCommandFilter == false) {
    ...
    ***REMOVED***
  ***REMOVED*** else if(readPreference != false) {
 ...
 *
 * @param {String***REMOVED*** collectionName the name of the collection we wish to drop.
 * @param {Function***REMOVED*** callback this will be called after executing this method. The first parameter will contain the Error object if an error occured, or null otherwise. While the second parameter will contain the results from dropCollection or null if an error occured.
 * @return {null***REMOVED***
 * @api public
 */
 ...
 * 
 * Options
 *  - **dropTarget** {Boolean, default:false***REMOVED***, drop the target name collection if it previously exists.
 *
 * @param {String***REMOVED*** fromCollection the name of the current collection we wish to rename.
 * @param {String***REMOVED*** toCollection the new name of the collection.
 * @param {Object***REMOVED*** [options] returns option results.
 * @param {Function***REMOVED*** callback this will be called after executing this method. The first parameter will contain the Error object if an error occured, or null otherwise. While the second parameter will contain the results from renameCollection or null if an error occured.
 * @return {null***REMOVED***
 * @api public
 */
 ...
 *
 * Options
 *  - **fsync** {Boolean, default:false***REMOVED***, option forces the database to fsync all files before returning.
 *  - **j** {Boolean, default:false***REMOVED***, awaits the journal commit before returning, > MongoDB 2.0.
 *  - **w** {Number***REMOVED***, until a write operation has been replicated to N servers.
 *  - **wtimeout** {Number***REMOVED***, number of miliseconds to wait before timing out.
 *
 * Connection Options
 *  - **connection** {Connection***REMOVED***, fire the getLastError down a specific connection.
 *
 * @param {Object***REMOVED*** [options] returns option results.
 * @param {Object***REMOVED*** [connectionOptions] returns option results.
 * @param {Function***REMOVED*** callback this will be called after executing this method. The first parameter will contain the Error object if an error occured, or null otherwise. While the second parameter will contain the results from lastError or null if an error occured.
 * @return {null***REMOVED***
 * @api public
 */
Db.prototype.lastError = function(options, connectionOptions, callback) {
  ...
  callback = args.pop();
  options = args.length ? args.shift() || {***REMOVED*** : {***REMOVED***;
  connectionOptions = args.length ? args.shift() || {***REMOVED*** : {***REMOVED***;
 ...
 *
 * @param {Object***REMOVED*** [options] returns option results.
 * @param {Function***REMOVED*** callback this will be called after executing this method. The first parameter will contain the Error object if an error occured, or null otherwise. While the second parameter will contain the results from previousErrors or null if an error occured.
 * @return {null***REMOVED***
 * @api public
 */
...
Db.prototype.executeDbCommand = function(command_hash, options, callback) {
  if(callback == null) { callback = options; options = {***REMOVED***; ***REMOVED***
 ...
 *
 * @param {Object***REMOVED*** [options] returns option results.
 * @param {Function***REMOVED*** callback this will be called after executing this method. The first parameter will contain the Error object if an error occured, or null otherwise. While the second parameter will contain the results from resetErrorHistory or null if an error occured.
 * @return {null***REMOVED***
 * @api public
 */
 ...
 *  - **fsync**, (Boolean, default:false) write waits for fsync before returning
 *  - **journal**, (Boolean, default:false) write waits for journal sync before returning
 *  - **unique** {Boolean, default:false***REMOVED***, creates an unique index.
 *  - **sparse** {Boolean, default:false***REMOVED***, creates a sparse index.
 *  - **background** {Boolean, default:false***REMOVED***, creates the index in the background, yielding whenever possible.
 *  - **dropDups** {Boolean, default:false***REMOVED***, a unique index cannot be created on a key that has pre-existing duplicate values. If you would like to create the index anyway, keeping the first document the database indexes and deleting all subsequent documents that have duplicate value
 *  - **min** {Number***REMOVED***, for geospatial indexes set the lower bound for the co-ordinates.
 *  - **max** {Number***REMOVED***, for geospatial indexes set the high bound for the co-ordinates.
 *  - **v** {Number***REMOVED***, specify the format version of the indexes.
 *  - **expireAfterSeconds** {Number***REMOVED***, allows you to expire data on indexes applied to a data (MongoDB 2.2 or higher)
 *  - **name** {String***REMOVED***, override the autogenerated index name (useful if the resulting name is larger than 128 bytes)
 * 
 * Deprecated Options 
 *  - **safe** {true | {w:n, wtimeout:n***REMOVED*** | {fsync:true***REMOVED***, default:false***REMOVED***, executes with a getLastError command returning the results of the command on MongoDB.
 *
 *
 * @param {String***REMOVED*** collectionName name of the collection to create the index on.
 * @param {Object***REMOVED*** fieldOrSpec fieldOrSpec that defines the index.
 * @param {Object***REMOVED*** [options] additional options during update.
 * @param {Function***REMOVED*** callback this will be called after executing this method. The first parameter will contain the Error object if an error occured, or null otherwise. While the second parameter will contain the results from createIndex or null if an error occured.
 * @return {null***REMOVED***
 * @api public
 */
  ...
  if(_hasWriteConcern(errorOptions) && typeof callback == 'function') {
    ...
    ***REMOVED***
    ...
    this._executeInsertCommand(command, commandOptions, function(err, result) {
      ...
      ***REMOVED***
    ***REMOVED***);
  ***REMOVED*** else if(_hasWriteConcern(errorOptions) && callback == null) {
 ...
 * @param {Object***REMOVED*** fieldOrSpec fieldOrSpec that defines the index.
 * @param {Object***REMOVED*** [options] additional options during update.
 * @param {Function***REMOVED*** callback this will be called after executing this method. The first parameter will contain the Error object if an error occured, or null otherwise. While the second parameter will contain the results from ensureIndex or null if an error occured.
 * @return {null***REMOVED***
 * @api public
 */
 ...
 *
 * @param {Object***REMOVED*** [options] additional options during update.
 * @param {Function***REMOVED*** callback this will be called after executing this method. The first parameter will contain the Error object if an error occured, or null otherwise. While the second parameter will contain the results from cursorInfo or null if an error occured.
 * @return {null***REMOVED***
 * @api public
 */
Db.prototype.cursorInfo = function(options, callback) {
  ...
  this._executeQueryCommand(DbCommand.createDbSlaveOkCommand(this, {'cursorInfo':1***REMOVED***)
    , options
 ...
 *
 * @param {String***REMOVED*** collectionName the name of the collection where the command will drop an index.
 * @param {String***REMOVED*** indexName name of the index to drop.
 * @param {Function***REMOVED*** callback this will be called after executing this method. The first parameter will contain the Error object if an error occured, or null otherwise. While the second parameter will contain the results from dropIndex or null if an error occured.
 * @return {null***REMOVED***
 * @api public
 */
 ...
 *
 * @param {String***REMOVED*** collectionName the name of the collection.
 * @param {Function***REMOVED*** callback this will be called after executing this method. The first parameter will contain the Error object if an error occured, or null otherwise. While the second parameter will contain the results from reIndex or null if an error occured.
 * @api public
**/
 ...
 *
 * Options
 *  - **full** {Boolean, default:false***REMOVED***, returns the full raw index information.
 *  - **readPreference** {String***REMOVED***, the preferred read preference ((Server.PRIMARY, Server.PRIMARY_PREFERRED, Server.SECONDARY, Server.SECONDARY_PREFERRED, Server.NEAREST).
 *
 * @param {String***REMOVED*** collectionName the name of the collection.
 * @param {Object***REMOVED*** [options] additional options during update.
 * @param {Function***REMOVED*** callback this will be called after executing this method. The first parameter will contain the Error object if an error occured, or null otherwise. While the second parameter will contain the results from indexInformation or null if an error occured.
 * @return {null***REMOVED***
 * @api public
 */
Db.prototype.indexInformation = function(collectionName, options, callback) {
  ...
  ***REMOVED***
  ...
  var selector = collectionName != null ? {ns: (this.databaseName + "." + collectionName)***REMOVED*** : {***REMOVED***;
 ...
 * Drop a database.
 *
 * @param {Function***REMOVED*** callback this will be called after executing this method. The first parameter will contain the Error object if an error occured, or null otherwise. While the second parameter will contain the results from dropDatabase or null if an error occured.
 * @return {null***REMOVED***
 * @api public
 */
 ...
 *
 * Options
 *  - **scale** {Number***REMOVED***, divide the returned sizes by scale value.
 *  - **readPreference** {String***REMOVED***, the preferred read preference ((Server.PRIMARY, Server.PRIMARY_PREFERRED, Server.SECONDARY, Server.SECONDARY_PREFERRED, Server.NEAREST).
 *
 * @param {Objects***REMOVED*** [options] options for the stats command
 * @param {Function***REMOVED*** callback this will be called after executing this method. The first parameter will contain the Error object if an error occured, or null otherwise. While the second parameter will contain the results from stats or null if an error occured.
 * @return {null***REMOVED***
 * @api public
 */
...
var __executeQueryCommand = function(self, db_command, options, callback) {
  ...
  ***REMOVED*** else if(typeof read == 'object' && read._type == 'ReadPreference') {
  ...
  ***REMOVED***
  ...
  if(typeof callback === 'function' && !onAll) {
    ...
    ***REMOVED*** else if(connection instanceof Error || connection['message'] != null) {
    ...
    ***REMOVED***
    ...
    connection.write(db_command, function(err) {
      ...
      ***REMOVED***
    ***REMOVED***);
  ***REMOVED*** else if(typeof callback === 'function' && onAll) {
    ...
    for(var i = 0; i < connections.length; i++) {
      ...
      if(connection == null) {
        return callback(new Error("no open connections"));
      ***REMOVED*** else if(connection instanceof Error) {
        return callback(connection);
      ***REMOVED***
...
Db.prototype._executeQueryCommand = function(db_command, options, callback) {
  ...
  ***REMOVED***
  ...
  if(!connection && !config.canRead(read) && !config.canWrite() && config.isAutoReconnect()) {
    ...
    ***REMOVED***
  ***REMOVED*** else if(!connection && !config.canRead(read) && !config.canWrite() && !config.isAutoReconnect()) {
...
Db.prototype._executeInsertCommand = function(db_command, options, callback) {
  ...
  ***REMOVED*** else if(!connection && !config.canWrite() && !config.isAutoReconnect()) {
    return callback(new Error("no open connections"), null);
  ***REMOVED*** else {
 ...
 *
 * Options
 *  - **uri_decode_auth** {Boolean, default:false***REMOVED*** uri decode the user name and password for authentication
 *  - **db** {Object, default: null***REMOVED*** a hash off options to set on the db object, see **Db constructor**
 *  - **server** {Object, default: null***REMOVED*** a hash off options to set on the server objects, see **Server** constructor**
 *  - **replSet** {Object, default: null***REMOVED*** a hash off options to set on the replSet object, see **ReplSet** constructor**
 *  - **mongos** {Object, default: null***REMOVED*** a hash off options to set on the mongos object, see **Mongos** constructor**
 *
 * @param {String***REMOVED*** url connection url for MongoDB.
 * @param {Object***REMOVED*** [options] optional options for insert command
 * @param {Function***REMOVED*** callback this will be called after executing this method. The first parameter will contain the Error object if an error occured, or null otherwise. While the second parameter will contain the db instance or null if an error occured.
 * @return {null***REMOVED***
 * @api public
 */
...
var _getWriteConcern = function(self, options, callback) {
  // Final options
  var finalOptions = {w:1***REMOVED***;
  // Local options verification
  if(options.w != null || typeof options.j == 'boolean' || typeof options.journal == 'boolean' || typeof options.fsync == 'boolean') {
    finalOptions = _setWriteConcernHash(options);
  ***REMOVED*** else if(options.safe != null && typeof options.safe == 'object') {
    finalOptions = _setWriteConcernHash(options.safe);
  ***REMOVED*** else if(typeof options.safe == "boolean") {
    finalOptions = {w: (options.safe ? 1 : 0)***REMOVED***;
  ***REMOVED*** else if(self.options.w != null || typeof self.options.j == 'boolean' || typeof self.options.journal == 'boolean' || typeof self.options.fsync == 'boolean') {
    finalOptions = _setWriteConcernHash(self.options);
  ***REMOVED*** else if(self.safe.w != null || typeof self.safe.j == 'boolean' || typeof self.safe.journal == 'boolean' || typeof self.safe.fsync == 'boolean') {
    finalOptions = _setWriteConcernHash(self.safe);
  ***REMOVED*** else if(typeof self.safe == "boolean") {
    finalOptions = {w: (self.safe ? 1 : 0)***REMOVED***;
  ***REMOVED***
