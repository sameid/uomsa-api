...
Request.prototype.getAgent = function (host, port) {
  if (!this.pool[host+':'+port]) {
    this.pool[host+':'+port] = new this.httpModule.Agent({host:host, port:port***REMOVED***)
  ***REMOVED***
...
Request.prototype.request = function () {
  ...
  ***REMOVED*** else if (self.jar) {
    // fetch cookie from the user defined cookie jar
    var cookies = self.jar.get({ url: self.uri.href ***REMOVED***)
  ***REMOVED*** else {
    // fetch cookie from the global cookie jar
    var cookies = cookieJar.get({ url: self.uri.href ***REMOVED***)
  ***REMOVED***
  if (cookies) {
  ...
  ***REMOVED***
  if (self.onResponse) self.on('error', function (e) {self.onResponse(e)***REMOVED***)
  if (self.callback) self.on('error', function (e) {self.callback(e)***REMOVED***)
  ...
  ***REMOVED***
  ...
  if (self.uri.auth && !self.headers.authorization) {
    self.headers.authorization = "Basic " + toBase64(self.uri.auth.split(':').map(function(item){ return qs.unescape(item)***REMOVED***).join(':'))
  ***REMOVED***
  if (self.proxy && self.proxy.auth && !self.headers['proxy-authorization']) {
    self.headers['proxy-authorization'] = "Basic " + toBase64(self.proxy.auth.split(':').map(function(item){ return qs.unescape(item)***REMOVED***).join(':'))
  ***REMOVED***
  ...
  ***REMOVED*** else if (self.multipart) {
    self.body = [];
  ...
  ***REMOVED***
  ...
  self.httpModule =
    {"http:":http, "https:":https***REMOVED***[self.proxy ? self.proxy.protocol : self.uri.protocol]
      ...
      if (response.statusCode >= 300 &&
        ...
        ***REMOVED***
        ...
        self.redirects.push( { statusCode : response.statusCode,
                               redirectUri: response.headers.location ***REMOVED***)
        delete self.req
        delete self.agent
...
function request (options, callback) {
  if (typeof options === 'string') options = {uri:options***REMOVED***
...
request.defaults = function (options) {
  var def = function (method) {
    var d = function (opts, callback) {
      if (typeof opts === 'string') opts = {uri:opts***REMOVED***
      for (var i in options) {
