 ...
 * @param {Function***REMOVED*** secret
 * @param {Function***REMOVED*** validate
 * @api public
 */
...
DigestStrategy.prototype.authenticate = function(req) {
  ...
  if (!authorization) { return this.fail(this._challenge()); ***REMOVED***
  
  var parts = authorization.split(' ')
  if (parts.length < 2) { return this.fail(400); ***REMOVED***
  
  var scheme = parts[0]
    , params = parts.slice(1).join(' ');
  
  if (!/Digest/i.test(scheme)) { return this.fail(this._challenge()); ***REMOVED***
  
  var creds = parse(params);
  if (Object.keys(creds).length === 0) { return this.fail(400); ***REMOVED***
  ...
  ***REMOVED***
  ...
  this._secret(creds.username, function(err, user, password) {
    if (err) { return self.error(err); ***REMOVED***
    if (!user) { return self.fail(self._challenge()); ***REMOVED***
    ...
    if (!creds.algorithm || creds.algorithm === 'MD5') {
      ...
      ***REMOVED***
    ***REMOVED*** else if (creds.algorithm === 'MD5-sess') {
    ...
    ***REMOVED*** else if (creds.qop === 'auth-int') {
    ...
    ***REMOVED*** else if (creds.qop === 'auth' || creds.qop === 'auth-int') {
      ...
      if (self._validate) {
        self._validate({
          ...
          ***REMOVED***,
          function(err, valid) {
            if (err) { return self.error(err); ***REMOVED***
            if (!valid) { return self.fail(self._challenge()); ***REMOVED***
            self.success(user);
          ***REMOVED***);
      ***REMOVED*** else {
        self.success(user);
